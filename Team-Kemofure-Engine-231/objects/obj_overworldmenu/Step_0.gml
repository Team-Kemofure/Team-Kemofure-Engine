...
switch (mainState) {
	...
	case 3:
		if (subState == 0) {
			...
			if (key_enter_press) {
				//cutscene_create(lang_raw(ds_list_find_value(contactList, selection)));
				show_debug_message(lang_array(ds_list_find_value(contactList, selection)));
				subState = 1;
			}
		}
		else {
			if (obj_overworldui.state == 0)
				active = false;
		}
		break;
}